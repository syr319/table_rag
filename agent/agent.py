...
class TableAgent:
    ...
    def solver_loop(self, df: pd.DataFrame, prompt: str) -> str:
        ...
        solution = ''
        init_prompt = prompt
        answer = None  # 核心修复：显式初始化 answer 为 None
        ...
        for i in range(self.max_depth):
            ...
            if self.is_terminal(text):
                n_iter = i + 1
                # 提取答案（优化条件判断，避免 None 报错）
                import re
                answer_match = re.search(r'final answer:\s*(.*?)\s*$', text.lower(), re.DOTALL)
                if answer_match:
                    answer = answer_match.group(1).strip()
                    if self.verbose:
                        print(f"✅ 提取到 Final Answer: {answer}")
                else:
                    answer = "No valid answer found"
                    if self.verbose:
                        print(f"⚠️  未提取到 Final Answer，设为默认值: {answer}")
                break
        ...
        # 兜底：若循环结束仍未提取到答案，显式赋值
        if answer is None:
            answer = "No valid answer found"
            if self.verbose:
                print(f"⚠️  推理结束未找到 Final Answer，设为默认值: {answer}")
