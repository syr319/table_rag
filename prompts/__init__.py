...
from prompts import tabfact, wtq, arcade,hitab,tablebench
...
def get_prompt_templates(task, agent_type):
    ...
    elif task == 'hitab' and 'TableRAG' in agent_type:
        return {
            'extract_column_prompt': hitab.tablerag_extract_column_prompt,
            'extract_cell_prompt': hitab.tablerag_extract_cell_prompt,
            'solve_table_prompt': hitab.tablerag_solve_table_prompt,
        }
    elif task == 'hitab' and agent_type in ['PyReAct', 'ReadSchema', 'RandSampling', 'TableSampling']:
        return {'solve_table_prompt': hitab.pyreact_solve_table_prompt}
    elif task == 'tablebench' and 'TableRAG' in agent_type:
        return {
            'extract_column_prompt': tablebench.tablerag_extract_column_prompt,
            'extract_cell_prompt': tablebench.tablerag_extract_cell_prompt,
            'solve_table_prompt': tablebench.tablerag_solve_table_prompt,
        }
    elif task == 'tablebench' and agent_type in ['PyReAct', 'ReadSchema', 'RandSampling', 'TableSampling']:
        return {'solve_table_prompt': tablebench.pyreact_solve_table_prompt}
